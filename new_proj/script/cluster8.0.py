...
a = '/home/chen/datasets/taking phone net filted last'  # 最后位置不带斜杠

folders = os.listdir(a)
print(folders)
for folder in folders:
    parts = os.listdir(a + '/' + folder)
    for part_num in parts:
        ...
        for clas in class_choice:
            for zero_one in range(2):
                # zero_one = 0
                # #zero_one = 1
                work_path = a + '/' + folder + '/' + part_num + '/' + clas
                # print(work_path)
                work_path_save = work_path + '/' + str(zero_one) + '/'
                print(work_path_save)
                all_pics = []
                        ...
                        for pic_list in reversed(all_pics):
                            if len(pic_list) > 0:
                                for img_path_pre in [pic_list[-1]]:
                                    ...
                                    hist_pre = cv2.calcHist([img_pre], [0, 1, 2], None, [128, 128, 128],
                                                            [0, 256, 0, 256, 0, 256])

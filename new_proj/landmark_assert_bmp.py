...
image_save_path = work_path1 + 'image_with_dot/'
if not os.path.exists(image_save_path):
    os.makedirs(image_save_path)

img_names = os.listdir(work_path)
#img_names = []
df = pd.read_csv(work_path1 + '01030010_flip_cut_aligned.csv', header=0, sep=', ', index_col='frame', engine='python')
#df = df.reindex(df['frame'])
print(df.head(1))
print(df.tail(1))
#print(type(df.loc[0, ['frame', 'frame_id']][0]))
#print((df.loc[0, ['frame', 'face_id']]))
print(list(df.loc[1, ['frame', 'face_id']]))
#print(df['frame'])
...
img_names = work_path.glob('*.bmp')
print(type(img_names))
for img_name in sorted(img_names):
    #print(type(img_name))
    #print(type(img_name.name))
    #print(img_name.name)
    img_path = str(img_name)    #work_path + str(img_name)
    image = cv2.imread(img_path)
    frame_num = int(float(str(img_name).split('.')[-2].split('_')[-1]))
    print(frame_num)
    landmarks_x = list(df.loc[frame_num, landmarks_name_x])
    landmarks_y = list(df.loc[frame_num, landmarks_name_y])
    data_others = list(df.loc[frame_num, data_names])
    #frame||confidence||success||yaw
    cv2.putText(image, img_name.name + '||' + str(data_others[0]) + '||' + str(data_others[1]) + '||' + str(data_others[2]), (10, 20),
